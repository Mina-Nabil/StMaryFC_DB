...
class ApiController extends Controller
{
    ...
    public function searchByName(Request $request)
    {
        ...
        if ($validation === true) {
            $users = User::with(['group', 'type', 'mainImage'])->join("groups", "groups.id", '=', 'USER_GRUP_ID')->where("GRUP_NAME",  "LIKE", "%" . $request->name . "%")->where("USER_NAME", "LIKE", "%" . $request->name . "%")->get('app_users.*');
        }
        if ($users) {
            return $this->getApiMessage(true, $users);
        } else
